...
Z = numpy.rot90(numpy.loadtxt('dane_1.txt'))
...
settings = {'name': 'wykres_1_2', 'title': '',
            'x_label': '$U_{in}$ [V]', 'y_label': '$S_v$', 'location': 'upper right'}

Plot_1D(X[:-1], Sv, [classic], settings, False, True).plot()

classic = {'color': 'orange', 'marker': 'd', 'markersize': 15, 'linestyle': 'None', 'linewidth': '5'}

settings = {'name': 'wykres_1', 'title': '',
            'x_label': '$I_z$ [mA]', 'y_label': '$U_z$ [V]', 'location': 'upper right'}

Plot_1D(Z[0], Z[-3], [classic], settings, False, True).plot()
...
settings = {'name': 'wykres_1_1', 'title': '',
