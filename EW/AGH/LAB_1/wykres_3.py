...
Z = numpy.rot90(numpy.loadtxt('dane_3.txt'))
...
dIo = [(b-a)/1000 for a, b in zip(X[:-1], X[1:])]
dUo = [(b-a) for a, b in zip(Y[:-1], Y[1:])]

Sv = [abs(b/a) for a,b in zip(dIo, dUo)]
print(Sv)
YY = prosta(x, *A)

classic = {'color': 'blue', 'marker': 'd', 'markersize': 15, 'linestyle': 'None', 'linewidth': '5'}

settings = {'name': 'wykres_3_2', 'title': '',
            'x_label': '$I_{out}$ [mA]', 'y_label': '$R_{out}$ $\Omega$', 'location': 'upper right'}

Plot_1D(Iin[:-1], Sv, [classic], settings, False, True).plot()

classic = {'color': 'orange', 'marker': 'd', 'markersize': 15, 'linestyle': 'None', 'linewidth': '5'}

settings = {'name': 'wykres_3', 'title': '',
            'x_label': '$I_{out}$ [mA]', 'y_label': '$\eta$ [%]', 'location': 'upper right'}

Plot_1D(Io, Ni, [classic], settings, False, True).plot()
...
settings = {'name': 'wykres_3_1', 'title': '',
            'x_label': '$I_{out}$ [mA]', 'y_label': '$U_z$ [V]', 'location': 'lower right'}
